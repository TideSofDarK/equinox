...
void UFDSaveGame::LoadGame(const UObject * WorldContextObject, TMap<FIntVector, ASurvivalItemBase*>& InventoryGrid, TMap<FIntVector, ASurvivalItemBase*>& StashGrid, int Slot, AActor* &EquippedItem)
{
	UFDSaveGame* LoadedGameInstance = nullptr;
	bool bIsPersistentSlot = Slot == UFDSaveGame::PersistentSlot;
	...
	if (IsValid(LoadedGameInstance))
	{
		...
		ApplyActorRecord(UGameplayStatics::GetPlayerPawn(WorldContextObject, 0), LoadedGameInstance->PlayerCharacterSaveData, bIsPersistentSlot);
